...
{
    public function penduduk_produktif()
    {
        $tgl_sekarang = date('Y-m-d');
        $tgl1 = date('Y-m-d', strtotime('-15 years', strtotime($tgl_sekarang)));
        $tgl2 = date('Y-m-d', strtotime('-64 years', strtotime($tgl1)));

        $penduduk_produktifs = penduduk::whereBetween('tanggal_lahir', [$tgl2, $tgl1])->orderBy('tanggal_lahir')->get();

        return view('laporan.penduduk_produktif', compact('penduduk_produktifs'));
    }
    ...
    public function cetakPendudukProduktif()
    {
        ...
        return view('laporan.cetakPendudukProduktif', compact('penduduk_produktifs'));
    }
    ...
    public function pendudukLevelPendidikan()
    {
        $jumlahPenduduks = nagari::join('jorong', 'jorong.nagari_id', '=', 'nagari.id')
            ...
            ->orWhere('level_pendidikan.nama', '=', 'SMP/Sederajat')
            ->groupBy('nagari.id')
            ->get();
